...
TEST (CeresSolver, HelloWorld1) {
    ...
    // я не вижу противоренияе функции и нуля в нуле, ведь у этой функции ноль в 5-ке, она спрого выпуклая и соответственно нуля в нуле не бывает.
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        T radius = ceres::sqrt(line[0] * line[0] + line[1] * line[1]);
        residual[0] = (line[0] * samplePoint[0] + line[1] * samplePoint[1] + line[2]) / radius;
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (int d = 0; d < 3; ++d) {
        ASSERT_NEAR(line_params[d] / line_params[2],
                    ideal_line[d] / ideal_line[2], threshold);
    ...
    // Ну это очень жёсткое требование, его можно сделать динамическим и тогда станет немного лучше
